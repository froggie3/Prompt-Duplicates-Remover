...
class PromptModifier
{
    ...
    public function array_duplicates(array $array): array
    {
        ...
        $formatted = array_count_values($array);

        $filtered = array_filter(
            $formatted,
            fn ($value) => ($value !== $NOEXISTS),
            ARRAY_FILTER_USE_BOTH
        );

        $duplicates = array_map(fn ($value) => --$value, $filtered);
     ...
     * @return array key = 値, value = 重複している回数
     */
    public function array_duplicates_old(array $array): array
    {
        $duplicates = [];
        $NOEXISTS = 1;
